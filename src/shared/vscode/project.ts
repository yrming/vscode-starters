...
import { CancellationTokenSource, ProgressLocation, Uri, window, workspace } from 'vscode'
...
import type { Logger, ProjectFolderSearchResults, StarterCreateTriggerData } from '../../types'
...
export async function getAllProjectFoldersAndExclusions(
  ...
  try {
    ...
    const projectFolders = await resultsPromise
    isComplete = true
...
export function getWorkspaceFolders(folder?: WorkspaceFolder) {
  if (!workspace.workspaceFolders)
    return []
  const flag = !((!folder || folder.uri.scheme !== 'file'))
