...
import type { Context } from './workspace'
...
export async function showInputBoxWithSettings(
  ...
  if (options.enableSetting) {
    input.buttons = [
      {
        iconPath: {
          dark: vs.Uri.file(context.asAbsolutePath('resources/settings.svg')),
          light: vs.Uri.file(context.asAbsolutePath('resources/settings.svg')),
        },
        tooltip: 'Settings',
      },
    ]
  ...
  switch (setting.settingKind) {
    ...
    case 'BOOL':
      const boolResult = await vs.window.showQuickPick(
        [
          { label: 'Yes' } as vs.QuickPickItem,
          { label: 'No' } as vs.QuickPickItem,
        ],
